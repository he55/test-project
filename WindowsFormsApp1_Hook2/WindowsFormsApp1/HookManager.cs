...
namespace PlayHooky {
  ...
  uint flProtect
	...
	public class HookManager {
		...
		public unsafe void Hook(MethodInfo original, MethodInfo replacement, MethodInfo oriReplacement) {
			...
			byte[] originalOpcodes = PatchJMP(original, replacement,oriReplacement);
		...
		private unsafe byte[] PatchJMP(MethodInfo original, MethodInfo replacement, MethodInfo oriReplacement) {

			//JIT compile methods
			RuntimeHelpers.PrepareMethod(original.MethodHandle);
			RuntimeHelpers.PrepareMethod(replacement.MethodHandle);
			RuntimeHelpers.PrepareMethod(oriReplacement.MethodHandle);
			...
			IntPtr oriReplacementSite = oriReplacement.MethodHandle.GetFunctionPointer();
			...
			unsafe {
				...
				uint oldProtecton2 = VirtualProtect(oriReplacementSite, (uint)offset*2, (uint) Natives.PageProtection.PAGE_EXECUTE_READWRITE);

                //get unmanaged function pointer to address of original site
                byte* originalSitePointer = (byte*) originalSite.ToPointer();
                byte* oriPointer = (byte*)oriReplacementSite.ToPointer();

                //copy the original opcodes
                for (int k = 0; k < offset; k++) {
					originalOpcodes[k] = *(originalSitePointer + k);
                    oriPointer[k] = *(originalSitePointer + k);
                }
				...
				if (is64) {
					...
					*(originalSitePointer + 5) = 0xC3;
                    ...
                    //push replacementSite
                    *(oriPointer+offset) = 0x68;
                    *((uint*)(oriPointer+offset + 1)) = (uint)originalSite.ToInt32()+6; //sets 4 bytes

                    //ret
                    *(oriPointer+offset + 5) = 0xC3;
                }
                ...
                //flush insutruction cache to make sure our new code executes
                FlushInstructionCache(oriReplacementSite, (uint)originalOpcodes.Length);

                //done
                VirtualProtect(oriReplacementSite, (uint)originalOpcodes.Length, oldProtecton2);
